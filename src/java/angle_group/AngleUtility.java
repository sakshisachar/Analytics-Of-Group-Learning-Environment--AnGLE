        ...
        if (person != null) {
            try {
                ...
                if (con != null) {
                    ...
                    //System.out.println("here is The "+pstmt);
        ...
        try {
            ...
            if (con != null) {
                ...
                while (res.next()) {
                    ...
                    ug.setParentId(res.getInt("parentId"));
                    int parentId = res.getInt("parentId");
                    pstmt3 = con.prepareStatement("select name from GroupInfo where groupId=?");
                    pstmt3.setInt(1,parentId);
                    res3 = pstmt3.executeQuery();
                    
                    if(res3.next())
                    {
                        ug.setParentName(res3.getString("name"));
                    }
 ...
 * @author Chirag & Sakshi
    ...
    public ArrayList<Topic> viewTopic(int uid, int gid) {
        ...
        try {
            ...
            if (con != null) {
                pstmt = con.prepareStatement("select parentId from GroupInfo where groupId=?");
                pstmt.setInt(1, gid);
                res = pstmt.executeQuery();
                if (res.next()) {
                    ...
                    res = pstmt.executeQuery();
                    if (res.next()) {
                        pstmt = con.prepareStatement("select * from topicList where group_id=? OR group_id =?");
                        pstmt.setInt(1, gid);
 ...
 * @author Chirag & Sakshi
    ...
    output :
        ...
        -1 - error
     */
   public int checkExistingGroup(String name) {
            int status = 0;
        try {
            if (con == null || con.isClosed()) {
                con = DBConnection.getDBConnection();
            ...
            if (con != null) {
                pstmt = con.prepareStatement("select * from GroupInfo where name = ?");
                pstmt.setString(1, name);
                res = pstmt.executeQuery();
...
//                    email check : true
    ...
    public String addNewProfile2(Profile p,int gid) {
        int result = 0;
        String lastMinute = "";
        try {
            if (con == null || con.isClosed()) {
            ...
            if (con != null) {
                ...
                res.next();
                lastMinute += res.getString("profileId");
                String profileIdTemp = res.getString("profileId");
                lastMinute += ":";
                lastMinute += res.getString("profileName");
                pstmt = con.prepareStatement("select profileIdList from GroupInfo where groupId=?");
                pstmt.setInt(1, gid);
                res = pstmt.executeQuery();
                String profileIdList1="";
                if(res.next())
                {
                    profileIdList1 = res.getString("profileIdList");
                    profileIdList1=profileIdList1+":"+profileIdTemp;
                    //System.out.println("profileIdList "+profileIdList1);
                }
                pstmt = con.prepareStatement("update GroupInfo set profileIdList=? where groupId=?");
                pstmt.setString(1, profileIdList1);
                pstmt.setInt(2,gid);
                //System.out.println("profilieId List Query"+pstmt);
                if(pstmt.executeUpdate()>0)                    
                {
                    lastMinute=String.valueOf(gid);
                }
    ...
    public int insert(AssignmentBean r) {
        ...
        if(status == 1 )
        {
            status=insertIntoTodoList(person.getLoginId(), r.getGroupid(), "Assignment", r.getTopicName(),now,d);
        }
        return status;
    ...
    public ArrayList<AssignmentBean> viewAssignmentSubmission(int gid, int userid) {
        ...
        try {
            ...
            if (con != null) {
                pstmt = con.prepareStatement("select * from group_assignment where gid = ? and uid <> ? order by id desc");
                pstmt.setInt(1, gid);
        ...
        try {
            ...
            if (con != null) {
                ...
                while (rs.next()) {
                    ...
                    pstmt2 = con.prepareStatement("select max(assignment_submission.grade) as highest from assignment_submission  where assignment_submission.aid=?");
                    pstmt2.setInt(1, b.getId());
                    res1 = pstmt2.executeQuery();
                    while (res1.next()) {
                        String foundType = res1.getString(1);
                        b.setGrade(res1.getString("highest"));
                        ps = con.prepareStatement("select assignment_submission.uid from assignment_submission  where assignment_submission.grade=? and assignment_submission.aid=?");
                        ps.setString(1, b.getGrade());
                        ps.setInt(2, b.getId());
                        res3 = ps.executeQuery();
                        while (res3.next()) {
                            if (b.getGrade() != null || b.getGrade() != "") {

                                b.setUid(res3.getInt("uid"));
...
/*
    ...
    public ArrayList<Notification> getNotificationForUser(int userId) {
        ...
        try {
            ...
            if (con != null) {
                pstmt = con.prepareStatement("select * from Notification where uid = ? ORDER BY dateTime DESC LIMIT 20");
                pstmt.setInt(1, userId);
 ...
 * To view all notes per group
   ...
   /*       
    ...
    public String saveQuizDetails(int userId, int groupId, String title, String dateStr, int duration) {
        ...
        int status = insertIntoTodoList(userId, groupId, "Quiz", title,parsedDate, parsedDate);
        return resultStr;
    }
    /*
    ...
    public String updateRoleOfMember(int gid, int uid, int pid) {
        ...
        String profileNameId=profileName+":"+String.valueOf(uid);
    ...
    public ToDoList getToDoList(int groupId) {
        ...
        try {
            ...
            if (con != null) {
                pstmt = con.prepareStatement("Select feature,startDate,endDate,topic from toDoList where groupId=?");
                ...
                while (res.next()) {
                    ...
                    if ((Integer.parseInt(arr[0]) - Integer.parseInt(year) >= 0) && (Integer.parseInt(arr[1]) - Integer.parseInt(month) < 6) && (Integer.parseInt(arr[1]) - Integer.parseInt(month) > 0)) {
                        ...
                        if (arrTemp != null) {
                            ...
                            topicToDo.setEndTime(res.getString("endDate"));
                            topicToDo.setStartTime(res.getString("startDate"));
...
//jforum
                for (j = 0; j < Features.size() - 1; j++) {
                    ...
                    featureList += ":";
                    //System.out.println("Features attributes :"+Features.get(j));
                }
                //System.out.println(featureList);
                if (j > 0) {
                    featureList += Features.get(j);
                ...
                if (responseId > 0) {
                    ...
                    while (res.next()) {
                        ...
                        if (rs2.next()) {
                            ...
                            while(rs2.next())
                            {
                                if(r.getSelectedOptionId() == r.getAnswerId())
                                {
...
//jforum
    ...
    public int getParentId(int gid)
    {   int pid=0;
       try {
            if (con == null || con.isClosed()) {
                con = DBConnection.getDBConnection();
    ...
    /*
        Function to get quiz report of a user
    */
    public QuizReport getQuizReport(Integer userId, Integer groupId)
    {
        Integer count=0,count2=0;
        QuizReport report=new QuizReport();
        ArrayList<Quiz> quizAttempted = new ArrayList<Quiz> ();
        ...
        try {
            ...
            if (con != null) 
            {
                ps=con.prepareStatement("select * from QuizData where groupId=?;");
                ps.setInt(1,groupId);
                rs=ps.executeQuery();
                while(rs.next())
                {
                    ...
                    pstmt=con.prepareStatement("select * from QuizResponse, QuizData where QuizData.quizId=? AND userId=? AND QuizResponse.quizId=QuizData.quizId;");
                    pstmt.setInt(1, quizId);
                    pstmt.setInt(2, userId);
                    res=pstmt.executeQuery();
                    Quiz q=new Quiz ();
                    if(res.next())
                    {
                        ...
                        q.setDuration(res.getInt("duration"));
                        q.setPassingMarks(res.getInt("marks"));
                        quizAttempted.add(q);
                    }
                }
                report.setQuizAttempted(quizAttempted);
                report.setTotalQuizCount(count);
                report.setQuizAttemptedCount(count2);
            }
        }
         catch (Exception e) {
            e.printStackTrace();
...
/*
    Gives analysis of the number for quizzes of that group attended by the user
    output :
        -1 - error in db connection
        any other non negative number - number of quizzes attended by the user in that group
    */
    public int userQuizAttendance(int groupId, int userId)
    {
        int noOfQuiz = -1;
        try {
            if (con == null || con.isClosed()) {
            ...
            if (con != null) {
                pstmt = con.prepareStatement("select count(quizId) as noOfQuiz from QuizResponse where userId = ? and quizId IN (select quizId from QuizData where groupId = ?)");
                pstmt.setInt(1, userId);
                pstmt.setInt(2, groupId);
                res = pstmt.executeQuery();
                if (res.next()) {
                    noOfQuiz = res.getInt("noOfQuiz");
                } else {
                    noOfQuiz = 0;
                }
            }
      ...
      /*
    Gives analysis of the total quiz marks of the user in that group
    output :
        -1 - error in db connection
        any other non negative number - total quiz marks of the user in that group
    */
    public int userTotalQuizMarks(int groupId, int userId)
    {
        int totalMarks = 0;
        try {
            if (con == null || con.isClosed()) {
            ...
            if (con != null) {
                pstmt = con.prepareStatement("select sum(marks) as totalMarks from QuizResponse where userId = ? and quizId IN (select quizId from QuizData where groupId = ?)");
                pstmt.setInt(1, userId);
                pstmt.setInt(2, groupId);
                res = pstmt.executeQuery();
                if (res.next()) {
    ...
    /*
    *   Function to get the count of quizzes of a group
    */
    public Integer getTotalQuiz(Integer groupId)
    {
        Integer total=0;
        try {
            if (con == null || con.isClosed()) {
            ...
            if (con != null) {
                pstmt = con.prepareStatement("select count(*) as total from QuizData where groupId = ? ;");
                pstmt.setInt(1, groupId);
                res = pstmt.executeQuery();
                if (res.next()) {
    ...
    /*
    *   Function to get the total marks of all the quizzes of a group
    */
    public Integer getTotalMarksByGroup(Integer groupId)
    {
        Integer total=0;
        try {
            if (con == null || con.isClosed()) {
            ...
            if (con != null) {
                pstmt = con.prepareStatement("select quizId from QuizData where groupId = ? ;");
                pstmt.setInt(1, groupId);
                res = pstmt.executeQuery();
                while (res.next()) {
                    ps=con.prepareStatement("select sum(marks) as total from QuizQuestion where quizId=?");
                    ps.setInt(1,res.getInt("quizId"));
                    rs=ps.executeQuery();
                    if(rs.next())
                    {
     ...
     public HashMap<Integer, HashMap<String,Integer>> getQuizAttendance( Integer groupId)
     {
        HashMap<Integer, HashMap<String,Integer>> quizAttendance = new HashMap<Integer, HashMap<String,Integer>>();
         try {
            if (con == null || con.isClosed()) {
                con = DBConnection.getDBConnection();
            }
            if (con != null) {
                pstmt = con.prepareStatement("select * from QuizData where groupId = ? ;");
                pstmt.setInt(1, groupId);
                res = pstmt.executeQuery();
                while (res.next()) {
                    HashMap<String, Integer> attendance= new HashMap<String, Integer> ();
                    ps=con.prepareStatement("select count(*) as total from QuizResponse where quizId=?");
                    ps.setInt(1,res.getInt("quizId"));
                    rs=ps.executeQuery();
                    if(rs.next())
                    {
                         attendance.put(res.getString("title"), rs.getInt("total"));
                         quizAttendance.put(res.getInt("quizId"), attendance);
                    }
                }
            }
        }
...
/*
    Gives min, max and avg marks of all quizzes in a group
    output :
        Arraylist of bean of AnalyseQuizMarks
    */
    public ArrayList<AnalyseQuizMarks> analyseQuizMark(int groupId)
    {
        ArrayList<AnalyseQuizMarks> quizMarksList = new ArrayList<AnalyseQuizMarks>();
        AnalyseQuizMarks aqm = null;
        try {
            if (con == null || con.isClosed()) {
                con = DBConnection.getDBConnection();
            }
            if (con != null) {
                pstmt = con.prepareStatement("select title, min(marks) as minimum, max(marks) as maximum, avg(marks) as average from QuizResponse, QuizData where QuizResponse.quizId=QuizData.quizId and QuizData.groupId = ? group by QuizResponse.quizId");
                pstmt.setInt(1, groupId);
                res = pstmt.executeQuery();
                while (res.next()) {
                    aqm = new AnalyseQuizMarks();
                    aqm.setMin(res.getInt("minimum"));
                    aqm.setMax(res.getInt("maximum"));
                    aqm.setAvg(res.getInt("average"));
                    aqm.setTitle(res.getString("title"));
                    quizMarksList.add(aqm);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
        ...
        return quizMarksList;
    }    
    
    /*
    Gives boolean value to describe if the user has attended the quiz or not
    output :
        true - not attended the quiz
        false - attended the quiz
    */
    public boolean attendedQuiz(int quizId, int userId)
    {
        try {
            if (con == null || con.isClosed()) {
                con = DBConnection.getDBConnection();
            ...
            if (con != null) {
                pstmt = con.prepareStatement("select * from QuizResponse where quizId = ? and userId = ?");
                pstmt.setInt(1, quizId);
                pstmt.setInt(2, userId);
                res = pstmt.executeQuery();
                if (res.next()) {
    ...
    /*
    *   Function to insert features in ToDo List
    */
     public Integer insertIntoTodoList(Integer userId,Integer groupId,String feature, String topic, Date startDate, Date endDate)
    {
        ...
        try {
            ...
            if (con != null) {
                pstmt = con.prepareStatement("INSERT INTO toDoList ( userId, groupId, feature, topic, startDate, endDate) values( ?, ?, ?, ?, ?, ?);");
                pstmt.setInt(1,userId);
                pstmt.setInt(2,groupId);
